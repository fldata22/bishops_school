    ...
    <PrincipalShell>
      <div className="px-4 md:px-8 pt-6 pb-12 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {classes.map(cls => {
            const cat = categoryLabel(cls.category)
            return (
              <Link
                key={cls.id}
                href={`/classes/${cls.id}`}
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[10px] font-label text-on-surface-variant/60 px-2 py-0.5 rounded-full border border-white/[0.07]"
                    style={{ background: 'rgba(255,255,255,0.04)' }}>
                    {studentCount(cls.id)} students
                  </span>
                  {cat && (
                    <span className="text-[10px] font-label text-primary-dim px-2 py-0.5 rounded-full border border-primary/20"
